...
Code snippet of the HelloWorld project is listed below.  There are only 5 lines required to run this program.
...
int main(void)
{
  ...
  uint16_t w, h;
  ssd7317_get_stringsize(&Tahoma_12h, "Hello World", &w, &h); //4
  ssd7317_put_string((OLED_HOR_RES-w)/2,(OLED_VER_RES-h)/2, &Tahoma_12h,"Hello World",0); //5
...
1. Functions `HAL_Init()` is automatically generated by **STM32CubeIDE** to reset all peripherals and initialize the Flash interface of the MCU. 

2. Function `SystemClock_Config()` is automatically generated by **STM32CubeIDE** to initialize the system clock of the MCU.

3. Function `ssd7317_init()` is the initialization function of SSD7317Z.

   Function `rpc_uart_init()` is the initialization function of the Remote Procedure Call module. This function is not required for SSD7317Z.

4. Function `ssd7317_get_stringsize(&Tahoma_12h, "Hello World", &w, &h)` returns the width and height of the string `Hello World` from the font `Tahoma_12h` defined in a header file **Tahoma_12h.h**. I will explain how to create the file in the section LCD Image Converter later.

5. Function `ssd7317_put_string((OLED_HOR_RES-w)/2,(OLED_VER_RES-h)/2, &Tahoma_12h,"Hello World",0)` does the job to align `Hello World` in the center of the screen, update the frame buffer, and get the string displayed on the next FR event.

   The function `rpc_main_task()` is not required for now. I leave it there for future experiments.

Pixels are not written directly to the screen. Instead, a frame buffer `uint8_t frame_buffer[OLED_VER_RES*(OLED_HOR_RES>>3)]` is devised to map the screen content of the OLED to SRAM of the MCU. The first data member `frame_buffer[0]` represents 8 pixels at the top left corner from COM95-COM88 at SEG127, `frame_buffer[1]` maps to COM87-COM80 at SEG127, etc. 

There is a memory region known as the **Graphic Display Data RAM (GDDRAM)** physically mapped to each pixel in an one-bit depth on the screen.
...
Figure below shows the SPI timing diagram. Data transmission occurs with DCS# goes low and data is latched on the rising edge of the serial clock with most significant bit (MSB) first. The signal D/C# controls whether the byte is a command (D/C#=0) or data (D/C#=1). 
...
```
To be honest with you, I did not invent the code myself that is mostly hardware dependent. If you double click on **HelloWorld.ioc** <img src ="./Images/Number_circle_44x44_1.png" width=28> from the Project Explorer to open the **Device Configuration Tool**, expand  **Connectivity : SPI1**<img src ="./Images/Number_circle_44x44_2.png" width=28>, you will find a more easy-to-understand table under the **Parameter Settings**<img src ="./Images/Number_circle_44x44_3.png" width=28>.

<img src ="./Images/HelloWorld_ioc.png" width=100%>

How major parameters are mapped to the SPI format are summarized in a table below.

| SPI format                                       | Parameters                                                   |
| ------------------------------------------------ | ------------------------------------------------------------ |
| Four lines required (D/C#, DCS#, SCLK, and SDIN) | Mode = Half-Duplex Master with Hardware NSS Output Signal    |
...
SPI command write is performed by the function `spi_write_command(const uint8_t *command, uint16_t len)` and all display parameters of the screen (e.g. segment address, contrast level, frame rate, etc.) are written by this function. However, SPI data write is slightly more complicated. Any byte transferred with a SPI-data-write (OLED_DC=1) is displayed as pixels as BLACK or WHITE across a certain page of 8-bit  (8-pixel) length in the COM direction.  Say, if we set the SEG address to 127 and COM address to 0 and write a value of 0xFF through SPI with OLED_DC driven high, all pixels across COM95:COM88 will be switched to WHITE color at the top left corner across COM95:COM88.

<img src="./Images/GDDRAM_closeup_COM95_88.png" width=100%>

This protocol causes an issue if we need to display pixels less than a multiple of 8 because the minimum transfer size is one byte (8 bits). 
