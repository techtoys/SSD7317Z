...
| <img src ="./Images/Number_circle_44x44_2.png" width="32"> | Touch-sensing driver is triggered by an internal circuitry of SSD7317Z in the blanking period |
...
The technique to drive display and touch sensing is called **Time Multiplex Driving Scheme**. In each display period there is a blanking period triggered by a rising FR pulse of 2.4ms (depends on display frame rate) during that period the SEG and COM layers are driven to sense capacitance fluctuation and SSD7317Z will generate a **Touch Report** if the fluctuation exceeds a certain threshold. There are two touch sensing modes: **Active Mode** and **Low Power Mode** with the major difference illustrated below.
...
In SSD7317Z, for touch screen there are only two registers to care about: 

1. **S&L** (Status and Length) at register address 0x0AF0

2. **Gesture Upload Data** at register address 0x0AF1

   Status and gesture data from these registers form the **Touch Report** that is generated on an FR pulse for a valid touch event. Its format and reading protocol is described in the following section.
...
**1.** Initialization starts with `MX_GPIO_Init()` in which GPIO setup for OLED_FR and OLED_DC for display is also performed. Next to the display part, GPIO setup for TCH_TRES (output push pull for hardware reset) and TCH_IRQ (external interrupt with falling edge trigger detection) are configured.
...
```
There are programmable I2C map addresses in SSD7317Z for touch screen: TOUCH_SA and TOUCH_SA_BIOS with _SA the short form of slave address.
...
**5.** Firmware download to 3 memory blocks of SSD7317Z: PM (12Kbytes), TM(2Kbytes), and DM(2Kbytes) to configure the number of *in-cell* and *out-cell* keys with firmware content delivered by Solomon Systech. There are follow-up procedures including checksum assertion, MCU un-stall (means to pass control to MCU), and interrupt event clear which are all encapsulated in the function `touch_init()`.
...
void ssd7317_init(void){
    ...
    /* initializing display part of the OLED omitted for simplicity */
...
> **NOTE:** Although it is stated on the datasheet that a 200us delay is mandatory between data packets in I2C write, actual experiments show that it is not essential. However, for i2c_read (see below) a packet delay of 200us is strictly required; otherwise, data integrity is corrupted.
...
**S&L** (Status and Length) at register address 0x0AF0:

During operation (after touch initialization), the **S&L** word (2 bytes) is returned on a valid touch event with content described in the figure below.
