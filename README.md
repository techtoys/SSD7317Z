...
> **NOTE:** All user code that do not go inside *USER CODE BEGIN xxx* and *USER CODE END xxx* markers will be removed on the next Code Generation if there is any change in Device Configuration Tool.
...
Interfacing touch screen of SSD7317Z is not much different than traditional *out-cell* touch. We still have to use I2C to read/write commands and assign GPIO for hardware reset and interrupt detection (refer to gray wirings of the interface diagram below). 
...
Pseudo code of the initialization sequence for touch screen is listed below as a summary.
...
During normal operation (after touch initialization), the **S&L** word (2 bytes) is returned on a valid touch event with content described in the figure below.
...
* up arrow for swipe up and down arrow for swipe down shown at upper left corner
...
| Marker                                                  | Descriptions                                                 |
| ------------------------------------------------------- | ------------------------------------------------------------ |
| <img src="./Images/Number_circle_44x44_1.png" width=30> | Function `HAL_Init()` is automatically generated by STM32CubeIDE to reset all peripherals and initialize the Flash interface of the MCU.  With a HAL_ prefix it means the function come from the hardware abstraction layer (HAL). No coding is required. |
...
| <img src="./Images/Number_circle_44x44_3.png" width=30> | This code initializes PA1 as a PWM generator to drive the buzzer onboard. On inspecting its content, you will find that the code is actually generated by STM32CubeIDE in `MX_TIM2_Init()`. Again, no coding is required. |
...
| <img src="./Images/Number_circle_44x44_3.png" width=30> | This section gets the label width for up to 4 digits as the background of the running number. We will use variable `label_bg` to clear the background of the label before updating a new value. |
