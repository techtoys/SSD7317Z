...
Traditional [OCA](https://en.wikipedia.org/wiki/Liquid_optically_clear_adhesive) bonded or air-gap touch screen stacks up additional layers on top of the LCD module to create a sensing surface for our fingers or stylus therefore extra thickness is added to the LCD module (figure on the left below). 

Novel *in-cell* touch screen assigns dual functions to SEG and COM layers for display and touch sensing. No extra ITO sensor layer is required. The Top Glass that usually forms the display window of the enclosure is bonded directly to the display with OCA (figure on the right below) therefore the total thickness is reduced.

<img src="./Images/Touchscreen_struct1.png" width=1153>

The technique to drive display and touch sensing is called **Time Multiplex Driving Scheme**. In each display period there is a blanking period triggered by a rising FR pulse of 2.4ms (depends on frame rate) during that period the SEG and COM layers are driven to sense capacitance fluctuation and SSD7317Z will generate a touch report if the fluctuation exceeds a certain threshold. There are two touch sensing modes: **Active Mode** and **Low Power Mode** with the major differences illustrated below.

<img src="./Images/Touchscreen_mode1.png" width=986>

SSD7317Z supports up to 4 *in-cell* touch keys and 4 *out-cell* touch keys and the number of keys is software configurable. On our evaluation board, the key map is illustrated in the figure below. 
...
**2.** The second step is to initialize I2C with `MX_I2C1_Init()` which is automatically generated by STM32CubeIDE. You may go back to the **HelloWorld** project, open **HelloWorld.ioc** and expand **Pinout & Configuration > Connectivity > I2C1** to inspect its parameter settings. No DMA or Interrupt (NVIC) setting is configured. 

**3.** The next step is to initialize SSD7317Z with a *magic* command sequence delivered by the manufacturer (Solomon Systech). It is stored in the static table `const uint8_t SSD7317_INIT_TBL[]` declared in `SSD7317.c` with an excerpt of the listing shown below. The upper part of `SSD7317_INIT_TBL[]` sets parameters for the display part and the part for touch screen starts with a command `0x31`:
...
```
There are two memory regions in SSD7317Z for touch screen: TOUCH_SA and TOUCH_SA_BIOS with _SA the short form of slave address. Slave addresses of SSD7317Z is programmable with different values of register `0x35` :
...
Full source code of `ssd7317_init()` is available from ../SSD7317Z/Drivers/SSD7317.c in this repository.
