...
Traditional [OCA](https://en.wikipedia.org/wiki/Liquid_optically_clear_adhesive) bonded or air-gap touch screen stacks up additional layers on top of the LCD module to create a sensing surface for our fingers or stylus. Extra thickness is required for the ITO sensing layer and an OCA layer that bonds the sensing surface to the display surface (figure on the left below). 

Novel *in-cell* touch screen assigns dual functions to SEG and COM layers: display and touch sensing. No extra ITO sensing layer is required. The Top Glass that usually forms the display window of the enclosure is bonded directly to the display with OCA (figure on the right below).

<img src="./Images/Touchscreen_struct1.png" width=1153>

The technique to drive display and touch sensing is called **Time Multiplex Driving Scheme**. In each display period there is a blanking period triggered by a rising FR pulse of 2.4ms during that period the SEG and COM layers are driven to sense capacitance fluctuation and SSD7317Z will generate a touch report if the fluctuation exceeds a certain threshold. There are two touch sensing modes: **Active Mode** and **Low Power Mode** with the major differences illustrated below.

<img src="./Images/Touchscreen_mode1.png" width=986>

SSD7317Z supports up to 4 *in-cell* touch keys and 4 *out-cell* touch keys and the number of keys is software configurable. On our evaluation kit, the key map is illustrated in the figure below. 
...
**1.** Initialization starts with `MX_GPIO_Init()` in which GPIO setup for OLED_FR and OLED_DC for display is also performed. For touch screen, GPIO setup for TCH_TRES (output push pull) and TCH_IRQ(external interrupt with falling edge trigger detection) are configured with interrupt enabled for TCH_IRQ that listens to a falling edge on IRQ output from the OLED module.

**2.** The second step is to initialize I2C with `MX_I2C1_Init()` which is automatically generated by STM32CubeIDE. You may go back to the **HelloWorld** project, open **HelloWorld.ioc** and expand **Pinout & Configuration > Connectivity > I2C1** and inspect its parameter settings. No DMA or Interrupt (NVIC) setting is configured. 

**3.** The next step is to initialize SSD7317Z with a *magic* command sequence delivered by the manufacturer (Solomon Systech). It is stored in the static table `const uint8_t SSD7317_INIT_TBL[]` declared in `SSD7317.c` with excerpt of the listing shown below. The upper part of `SSD7317_INIT_TBL[]` sets parameters for the display part and the part for touch screen starts with a command `0x31`:
...
```
There are two memory regions in SSD7317Z for touch screen: TOUCH_SA and TOUCH_SA_BIOS with _SA the short form of slave address. Slave addresses of SSD7317Z is programmable with different values set to `0x35` :
...
Compile-time directive `USE_TOUCH_SA_SET_A` is to control which option is applied. If there is no I2C address conflict with option A in your system, leaving `#define USE_TOUCH_SA_SET_A ` in **SSD7317.h** is fine. 

**4.** Drive TRES pin low for > 10us to reset the touch screen.

**5.** Firmware download to 3 memory blocks of SSD7317Z: PM (12Kbytes), TM(2Kbytes), and DM(2Kbytes) to configure the number of *in-cell* and *out-cell* keys. The content of the firmware is delivered by Solomon Systech. There are follow-up procedures including checksum assertion, MCU un-stall (means to pass control to MCU), and interrupt event clear which are all encapsulated in the function `touch_init()`.
...
void ssd7317_init(void){
    /* initializing display part of the OLED */
    gpio_init();//GPIO initialization for TRES and IRQ for interrupt detection
    i2c_init(); //I2C initialization
    spi_write_command((const uint8_t*)SSD7317_INIT_TBL, byte_count); //Send command table
    TRES_PIN_high_low_high(); //TRES pin driven low for >10us to hardware reset
    firmware_download(); //Firmware download to PM, TM, and DM regions in touch_init()
}
```

Full source code of `ssd7317_init()` is available from ../SSD7317Z/Drivers/SSD7317.c of this repository.

### Reading the Touch Report

Low level I2C read and write protocols for SSD7317Z conform to standard I2C operations. The read and write operations are illustrated with figures and correlation with their code listing below:
...
{
    if(len){
		//swap high and low bytes so that lower byte is sent first for ADD[7:0] and ADD[15:8] in the diagram
		uint16_t reg_byte_swap = ((reg<<8)&0xff00) | ((reg>>8)&0x00ff);
        //HAL_ is the hardware dependent function to write data with I2C
		HAL_I2C_Mem_Write(&hi2c1, slave<<1, reg_byte_swap, 2, (uint8_t *)data, len, 5000);
	}
	else
	{
        //HAL_ is the hardware dependent function to write to a register at a slave address with no param
		HAL_I2C_Master_Transmit(&hi2c1, slave<<1, (uint8_t *)&reg, 2, 500);
	}
}
```

> **NOTE:** Although it is stated on the datasheet that a 200us is mandatory between data packets in I2C write, actual experiments show that it is not essential. However, for i2c_read (see below) a packet delay of 200us is mandatory.
...
**There is only one register (0x0AF0) to read for the Touch Report.**
