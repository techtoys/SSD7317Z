...
Listing below shows a code snippet of the HelloWorld project.  There are only 5 lines required to run this program.
...
3. `ssd7317_init()` is the initialization function of SSD7317Z.
   ...
   `rpc_uart_init()` is an optional function to initialize the Remote Procedure Call (RPC) module that allows us to input commands from a serial terminal to control the OLED.
...
5. `ssd7317_put_string((OLED_HOR_RES-w)/2,(OLED_VER_RES-h)/2, &Tahoma_12h,"Hello World",0)` does the job to align `Hello World` in the center of the screen, update the frame buffer, and get the string displayed on the next FR event.

   The function `rpc_main_task()` is the API for RPC. I leave it there for future experiments.
